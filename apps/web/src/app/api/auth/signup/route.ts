...
export async function POST(req: NextRequest) {
  try {
    ...
    const upstream = await fetch(`${METEOR_API_URL}/api/v1/auth/signup`, {
      method: "POST",
    ...
    if (!upstream.ok) {
      logError("auth.signup.fail", { status: upstream.status, error: data.error });
    ...
    logInfo("auth.signup.ok", { userId: data.user?._id });
    return res;
  } catch (error) {
